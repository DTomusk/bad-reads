...
from src.bookclubs.api.dependencies import create_meeting_use_case, get_book_clubs_use_case, get_meetings_by_book_club_use_case
from src.books.api.dependencies import get_book_details_use_case
...
@router.get("/")
async def get_book_clubs(get_book_clubs=Depends(get_book_clubs_use_case), page: int = 1, page_size: int = 10, sort_by: str = "title", sort_order: str = "asc", author_id: UUID = None):
    """
    Get all book_clubs.
    """
    book_clubs = get_book_clubs.execute(page, page_size, sort_by, sort_order, author_id)
    return book_clubs

@router.get("/{book_club_id}")
async def get_book_details(book_club_id: UUID, get_book_club_details=Depends(get_book_details_use_case)):
    """
...
async def new_book_club_meeting(
    ...
    """
    new_meeting.execute(book_club_id=book_club_id)
