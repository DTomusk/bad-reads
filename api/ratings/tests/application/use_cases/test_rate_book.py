...
def test_rate_book_creates_new_rating(
    ...
    # Act
    use_case.execute(book_id, user_id, 4.5)
    ...
    mock_book.rate.assert_called_once_with(user_id, 4.5)
    mock_book_repository.update_book.assert_called_once_with(mock_book)

def test_rate_book_updates_existing_rating(
    mock_book_repository, mock_rating_repository, book_id, user_id, mock_book
):
    # Arrange
    use_case = RateBook(mock_book_repository, mock_rating_repository)
    existing_rating = MagicMock(spec=Rating)
    mock_rating_repository.get_rating_by_user_and_book.return_value = existing_rating

    # Act
    use_case.execute(book_id, user_id, 3.0)

    # Assert
    assert existing_rating.rating == 3.0
    mock_rating_repository.update_rating.assert_called_once_with(existing_rating)
    mock_rating_repository.create_rating.assert_not_called()
    mock_book.rate.assert_called_once_with(user_id, 3.0)
    mock_book_repository.update_book.assert_called_once_with(mock_book)
