...
export default function Register() {
  ...
  const { showErrorAlert, errorMessage, handleError, clearError } =
    useApiErrorHandler();
  ...
  useEffect(() => {
    const hasEmptyFields = Object.values(form.values).some(
      (val) => !val.trim()
    );
  ...
  return (
    <Center>
      <form onSubmit={form.onSubmit(handleSubmit)}>
        <Stack>
          <Title ta="center" order={1} mb="sm" mt="xl">
            ✨Register✨
          </Title>
          {showSuccessAlert && (
            <AlertBanner
              ...
              type="success"
              message="Welcome to the coolest club in town"
            ></AlertBanner>
          )}
          {showErrorAlert && (
            <AlertBanner
